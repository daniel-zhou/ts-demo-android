...
$.localise = function(packages, settings, loadBase, path, timeout) {
    ...
    var opts = {async: false, dataType: 'script', cache: true,
        timeout: (settings.timeout != null ? settings.timeout : 500)};
    var localiseOne = function(langpackage, lang) {
        if (settings.loadBase) {
            $.ajax($.extend(opts, {url: paths[0] + langpackage + '.js'}));
        }
        if (lang.length >= 2) {
            $.ajax($.extend(opts, {url: paths[1] + langpackage + '_' + lang.substring(0, 2) + '.js'}));
        }
        if (lang.length >= 5) {
            $.ajax($.extend(opts, {url: paths[1] + langpackage + '_' + lang.substring(0, 5) + '.js'}));
        }
    };
...
/* Ensure language code is in the format aa_bb. */
function normaliseLang(lang) {
    lang = lang.replace(/_/, '-').toLowerCase();
    if (lang.length > 3) {
        lang = lang.substring(0, 3) + lang.substring(3).toLowerCase();
    }
    return lang;
}
